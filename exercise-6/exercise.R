...
female.names <- read.csv("~/Desktop/INFO 201/m8-dataframes/exercise-6/data/female_names.csv", 
...
MostPopular <- function(my.year){
  names.x <- name[year==my.year]
  prop.x <- prop[year == my.year]
  popular.name <- names.x[prop.x==max(prop.x)]
  return(popular.name)
}
...
HowPopular <- function(name, my.year){
  names.x <- name[year==my.year]
  prop.x <- prop[year == my.year]
  popular.name <- names.x[prop.x==max(prop.x)]
